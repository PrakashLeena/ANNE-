    ...
    <div className={`cc ${sel ? 'sel' : ''}`} style={{ display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ padding:'13px', flex:1, display:'flex', flexDirection:'column' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:5 }}>
          ...
          <span style={{ fontSize:'.55rem', textTransform:'uppercase', background:'rgba(17,101,255,.10)', color:'var(--primary)', padding:'2px 7px', borderRadius:'9999px', whiteSpace:'nowrap', marginLeft:7, flexShrink:0 }}>{comp.tag}</span>
        </div>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <button onClick={() => toggle(comp.id)} style={{ background: sel ? 'rgba(17,101,255,.12)' : 'rgba(17,101,255,.10)', color: sel ? 'var(--primary)' : 'var(--primary)', border: sel ? '1px solid rgba(17,101,255,.28)' : '1px solid rgba(17,101,255,.22)', padding:'5px 13px', borderRadius:'9999px', fontSize:'.72rem', fontWeight:600, cursor:'pointer', transition:'all .2s', fontFamily:'Inter' }}>
            {sel ? '✓ Added' : '✦ Select'}
          </button>
        </div>
      </div>
    ...
    <div style={{ maxWidth:1280, margin:'0 auto', padding:'56px 32px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:28, alignItems:'start' }}>
              ...
              return (
                <div key={cat.k}>
                  <div style={{ display:'flex', alignItems:'center', gap:11, marginBottom:16 }}>
                    <div style={{ width:32, height:32, borderRadius:'.5rem', background:'rgba(17,101,255,.10)', border:'1px solid rgba(17,101,255,.22)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'.95rem' }}>{cat.i}</div>
        ...
        <div className="sidebar-sticky">
          ...
          {selected.size === 0 ? (
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
              {[...selected].map(id => {
                ...
                if (!c) return null;
                return (
                  <div style={{ background:'rgba(17,101,255,.075)', border:'1px solid rgba(17,101,255,.20)', borderRadius:'.5rem', padding:'9px 13px', display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:'.78rem' }}>
          ...
          {!user && (
            <input style={{ width:'100%', background:'var(--surface)', border:'1px solid var(--outline)', borderRadius:'8px', padding:'11px 14px', color:'var(--on-surface)', fontFamily:'Inter', fontSize:'.8rem', outline:'none', marginTop:14, transition:'border-color .2s' }}
              type="email" placeholder="Your email address" value={email} onChange={e => setEmail(e.target.value)} />
          )}
          <textarea style={{ width:'100%', background:'var(--surface)', border:'1px solid var(--outline)', borderRadius:'8px', padding:'11px 14px', color:'var(--on-surface)', fontFamily:'Inter', fontSize:'.8rem', outline:'none', marginTop:10, resize:'vertical', minHeight:72, transition:'border-color .2s' }}
